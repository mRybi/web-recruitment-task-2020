...
describe("Cart component", () => {
  ...
  test("displays proper child component", () => {
    const { queryByText, rerender } = renderWithRedux(<Cart />, {
      initialState: { cartItems: [...cartItems] },
    });

    let emptyMessage = queryByText(/Your cart is empty!/i);
    ...
    emptyMessage = queryByText(/Your cart is empty!/i);

    expect(emptyMessage).toBeInTheDocument();
  });

  test("displays proper subtotal value for items", async () => {
    const { rerender, getByTestId } = renderWithRedux(<Cart />, {
      initialState: { cartItems: [] },
    });
    let subtotal = getByTestId("subtotal");
    expect(subtotal).toBeInTheDocument();
    expect(subtotal.innerHTML).toBe(priceFormatter("0.00"));
    ...
    let expectedSubtotalValue = cartItems
      .reduce((sum, item) => sum + item.price, 0)
      .toFixed(2);
    expect(subtotal.innerHTML).toBe(priceFormatter(expectedSubtotalValue));

    renderWithRedux(
      <Cart />,
      {
        initialState: { cartItems: [...cartItemsDic] },
      },
      rerender
    );

    expectedSubtotalValue = cartItemsDic
      .reduce((sum, item) => sum + item.price, 0)
      .toFixed(2);
  ...
  test("displays proper discount value for items", async () => {
    const { rerender, getByTestId } = renderWithRedux(<Cart />, {
      initialState: { cartItems: [] },
    });
    let discount = getByTestId("discount");
    expect(discount).toBeInTheDocument();
    expect(discount.innerHTML).toBe(priceFormatter("0.00"));
    ...
    const expectedDiscountValue = cartItemsDic.sort((a, b) =>
      a.price > b.price ? 1 : -1
    )[0];
    expect(discount.innerHTML).toBe(
      priceFormatter(expectedDiscountValue.price)
    );
  });

  test("displays proper total value for items", async () => {
    const { rerender, getByTestId } = renderWithRedux(<Cart />, {
      initialState: { cartItems: [] },
    });
    ...
    expect(total.innerHTML).toBe(priceFormatter("0.00"));
    ...
    let totalValue = cartItems
      .reduce((sum, item) => sum + item.price, 0)
      .toFixed(2);

    expect(total.innerHTML).toBe(priceFormatter(totalValue));
    ...
    let discount = cartItemsDic.sort((a, b) => (a.price > b.price ? 1 : -1))[0]
      .price;
    totalValue = cartItemsDic
      .reduce((sum, item) => sum + item.price, 0)
      .toFixed(2);

    expect(total.innerHTML).toBe(
      priceFormatter((totalValue - discount).toFixed(2))
    );

    renderWithRedux(
      <Cart />,
      {
        initialState: { cartItems: [...cartItemsdMixed] },
      },
      rerender
    );

    discount = cartItemsdMixed.sort((a, b) => (a.price > b.price ? 1 : -1))[0]
      .price;
    totalValue = cartItemsdMixed
      .reduce((sum, item) => sum + item.price, 0)
      .toFixed(2);
