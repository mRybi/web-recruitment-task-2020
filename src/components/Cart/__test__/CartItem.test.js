...
describe("Cart component", () => {
  test("renders correctly and function is fired once", () => {
    const handleRemoveMock = jest.fn();
    const { getByTestId } = render(
      <CartItem item={cartItem} onRemove={handleRemoveMock} />
    );

    const renderedPrice = getByTestId("price");
    const renderedTitle = getByTestId("title");

    expect(renderedPrice.innerHTML).toBe(priceFormatter(cartItem.price));
    expect(renderedTitle.innerHTML).toBe(cartItem.title);
