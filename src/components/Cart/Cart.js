...
import { removeFromCart } from '../../actions/getBooks';

const calculateDiscount = items => {
    const specialOfferItems = items.filter(item => item.specialOffer);
    const sortedByPrice = specialOfferItems.sort((a, b) => a.price > b.price ? 1 : -1);
    
    return sortedByPrice.slice(0, Math.floor(specialOfferItems.length/3))
        ...
        <div className="tile is-parent">
            <div className="tile is-child box">
                ...
                {cartRedux.length === 0 ? (
                    ...
                    <CartItemsList items={cartRedux} onRemoveItem={(isbn) => dispatch(removeFromCart(isbn))} />
                )}
